...
def _has_fatal_error(namespace: str, pods: list) -> tuple[bool, str]:
    ...
    for p in pods:
        ...
        if status == "Pending":
            if raw_reason in ("ImagePullBackOff", "ErrImagePull"):
                return True, f"Pod '{name}': {raw_reason} — Docker Hub pull failed. Check your network connection."
...
def _watch_airflow(namespace: str, timeout_seconds: int = 1200):
    ...
    with Live(console=console, refresh_per_second=0.5, transient=False) as live:
        while True:
            ...
            # Fetch cluster state exactly once per loop iteration
...
def _fix_statefulset_conflict(namespace: str = "airflow"):
    ...
    console.print(f"[cyan]  Deleting {len(statefulsets)} StatefulSet(s)...[/cyan]")
    del_result = _run(
    ...
    if del_result.returncode == 0:
        console.print(f"[green]  ✓  Deleted StatefulSet(s).[/green]")
    else:
    ...
    if pvcs:
        ...
        # Step B: batched delete completes immediately (no hang)
        _run(
