...
def _fix_statefulset_conflict(namespace: str = "airflow"):
    ...
    del_result = _run(
        ["kubectl", "delete", "statefulset", "-n", namespace,
         ...
         "--ignore-not-found"] + statefulsets,
        check=False, capture=True
    )
    if del_result.returncode == 0:
        console.print(f"[green]  ✓  Deleted StatefulSet(s): {', '.join(statefulsets)}[/green]")
    else:
        console.print(f"[yellow]  ⚠  Could not fully delete StatefulSet(s): {del_result.stderr.strip()[:80]}[/yellow]")
    ...
    if pvcs:
        ...
        _run(
            ["kubectl", "delete", "pvc", "-n", namespace,
             "--ignore-not-found", "--timeout=15s"] + pvcs,
            check=False, capture=True
        )
