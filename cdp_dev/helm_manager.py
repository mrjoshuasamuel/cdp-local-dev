...
def _fix_statefulset_conflict(namespace: str = "airflow"):
    ...
    # ⚡ Bolt: Batched multiple `kubectl delete statefulset` operations into a
    # single subprocess call to significantly reduce execution time.
    del_result = _run(
    ...
    if del_result.returncode == 0:
        console.print("[green]  ✓  Deleted StatefulSets.[/green]")
    else:
    ...
    if pvcs:
        ...
        # ⚡ Bolt: Batched multiple `kubectl delete pvc` operations into a
        # single subprocess call to reduce I/O overhead.
        _run(
